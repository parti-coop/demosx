...
CREATE TABLE `TB_ISSUE_CATEGORY` (
	`CATE_ID`   BIGINT(20) UNSIGNED NOT NULL COMMENT '범주ID', -- 범주ID
	`CATE_NAME` VARCHAR(100)        NOT NULL COMMENT '범주명', -- 범주명
...
CREATE TABLE `TB_OPINION` (
	...
	`OPIN_COMMENT`   VARCHAR(3000)       NOT NULL COMMENT '의견댓글' -- 의견댓글
)
COMMENT '의견';
...
-- 이슈첨부파일
CREATE TABLE `TB_ISSUE_ATTACHEMENT` (
	`ISSUE_ID`  BIGINT(20) UNSIGNED NOT NULL COMMENT '이슈ID', -- 이슈ID
...
-- 이슈첨부파일
ALTER TABLE `TB_ISSUE_ATTACHEMENT`
	ADD CONSTRAINT `PK_TB_ISSUE_ATTACHEMENT` -- 이슈첨부파일 Primary key
		PRIMARY KEY (
			`ISSUE_ID`, -- 이슈ID
			`FILE_SEQ`  -- 파일순번
		);
...
-- 이슈첨부파일
ALTER TABLE `TB_ISSUE_ATTACHEMENT`
	ADD CONSTRAINT `FK_TB_ISSUE_TO_TB_ISSUE_ATTACHEMENT` -- 이슈 -> 이슈첨부파일
		FOREIGN KEY (
			`ISSUE_ID` -- 이슈ID
		)
		REFERENCES `TB_ISSUE` ( -- 이슈
	...
	ADD INDEX `FK_TB_ISSUE_TO_TB_ISSUE_ATTACHEMENT` (
		`ISSUE_ID` ASC -- 이슈ID
	);
